...
export function createVortexStore(
  sanityCallback: (err: StateError) => void,
): Redux.Store<IState> {
  const middleware = [thunkMiddleware, reduxSanity(sanityCallback)];

  const enhancer: Redux.StoreEnhancer<IState> = compose(
    applyMiddleware(...middleware, forwardToRenderer),
  ) as Redux.StoreEnhancer<any>;

  const store = createStore<IState, Redux.Action, any, any>(
    reducer([], querySanitize, (error) =>
      log("error", "Failed to update application state", error.message),
    ),
    enhancer,
  );
