...
describe('UnixResolver', () => {
  ...
  describe('parent delegation', () => {
    test('delegates unknown anchors to parent', async () => {
      ...
      class MockResolver extends UnixResolver {
        constructor() {
          super();
          this.name = 'mock';
        }
      ...
      const unixWithParent = new UnixResolver(mockParent);

      // Root anchor handled by UnixResolver
      const root = unixWithParent.PathFor('root');
      expect(await root.resolve()).toBe('/');

      // Mock anchor delegated to parent
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const mockPath = unixWithParent.PathFor('mock' as any);
