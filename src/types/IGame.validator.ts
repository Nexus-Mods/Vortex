...
export const isIGame = ajv.compile(IGameSchema) as ValidateFunction<IGame>;
export default function validate(value: unknown): IGame {
  if (isIGame(value)) {
    return value;
  } else {
    throw new Error(
      ajv.errorsText(isIGame.errors!.filter((e: any) => e.keyword !== 'if'), {dataVar: 'IGame'}) +
      '\n\n' +
      inspect(value),
    );
  }
