...
const getElectronPath = (() => {
  if (electron && electron.app) {
    return makeRemoteCallSync(
      "get-electron-path",
      (electronIn, webContents, id: string) => {
        if (!electronIn.app) {
          throw new Error(
            "Electron app is not available. This code must run in the Electron main process.",
          );
        }
        if (id === "__app") {
...
const setElectronPath = makeRemoteCallSync(
  "set-electron-path",
...
export async function getVortexPathAsync(id: AppPath): Promise<string> {
  ...
  try {
    ...
    const getElectronPathRemote = makeRemoteCall(
      "get-electron-path",
      (electronIn, webContents, id: string) => {
