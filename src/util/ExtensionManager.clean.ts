...
class ContextProxyHandler implements ProxyHandler<any> {
   ...
   * This function is only available during the init phase.
   * @param {string} extensionPath - path to the extension
   * @param {string} extensionName - name of the extension
   * @param {boolean} dynamic - whether the extension is dynamic (can be enabled/disabled)
   * @param {ExtensionInit} initFunc - the function to initialize the extension
   * @returns {void}
   */
  public registerExtension(extensionPath: string, extensionName: string, dynamic: boolean,
                          initFunc: () => ExtensionInit) {
    if (!this.mMayRegister) {
      log('warn', 'extension tries to register another extension outside init function', {
        extension: this.mCurrentExtension,
    ...
    this.mInitCalls.push({
      ...
      arguments: [extensionPath, extensionName, dynamic, initFunc],
      optional: false,
    });
  }
...
class ExtensionManager {
  ...
  private mRepositoryLookup: Array<{ [repository: string]: IRepositoryLookup }> = [] as Array<{ [repository: string]: IRepositoryLookup }>;
  private mArchiveHandlers: Array<{ [extension: string]: ArchiveHandlerCreator }> = [] as Array<{ [extension: string]: ArchiveHandlerCreator }>;
  ...
  private mExtensionState: Array<{ [extId: string]: IExtensionState }> = [] as Array<{ [extId: string]: IExtensionState }>;
  private mLoadFailures: { [extId: string]: IExtensionLoadFailure[] } = {} as { [extId: string]: IExtensionLoadFailure[] };
  private mOptionalExtensions: { [extId: string]: IExtensionOptional[] } = {} as { [extId: string]: IExtensionOptional[] };
  ...
  private mProgrammaticMetaServers: { [id: string]: modmetaT.IServer } = {} as { [id: string]: modmetaT.IServer };
  ...
  private mFailedWatchers: Set<string> = new Set<string>();
  ...
  public setStore<S extends IState>(store: ThunkStore<S>) {
    ...
    this.mExtensionState = [getSafe(store.getState(), ['app', 'extensions'], {})] as Array<{ [extId: string]: IExtensionState }>;
  ...
  public doOnceMain(): Promise<void> {
    const onceCallbacks = this.mContextProxyHandler.getCalls('onceMain');

    return promiseMapSeries(onceCallbacks, (call) => {
      // TODO: Implement proper queue logic here
      return Promise.resolve(call.arguments[0]())
        .then(() => {
          log('debug', 'once done');
        })
        .catch(err => {
        ...
        .catch(err => {
          if (err.code === 'EACCES') {
            // this is a windows-ism, can't access a directory due to access rights
            //  set by the directory owner. It's the same as EPERM but more specific
            return Promise.reject(new ProcessCanceled('Directory access forbidden by OS'));
          } else {
            return Promise.reject(err);
          }
        })
  ...
  private emitAndAwait = (event: string, ...args: any[]): Promise<any> => {
    return new Promise((resolve, reject) => {
      this.mApi.events.emit(event, ...args, (err, res) => {
        if (err !== undefined) {
          reject(err);
        } else {
          resolve(res);
    ...
    return preMD5.then((results: IModLookupResult[]) => {
      ...
      if (results.length !== 0) {
        ...
        return this.getModDB()
          ...
          .then((mods: ILookupResult[]) => promiseFilter(mods, (mod: ILookupResult) => {
            if (options.requireURL === true) {
    
              return Promise.resolve(truthy(mod.value.sourceURI));
            } else {
              return Promise.resolve(true);
            }
          }))
          .then((filteredMods: ILookupResult[]) => promiseMap(filteredMods, (mod: ILookupResult) => Promise.resolve(convertMD5Result(mod))));
      }
    })
  ...
  private applyStartHooks = (input: IRunParameters): Promise<IRunParameters> => {
    let updated = input;
    return promiseEach(this.mStartHooks, (hook) => hook.hook(updated)
  ...
  private runExecutable =
    (executable: string, args: string[], options: IRunOptions): Promise<void> => {
     ...
     return Promise.resolve(null); } else { return Promise.reject(err); }})
        .catch(err => { if (err.code === 'EACCES') {
     return this.runElevated(executable, cwd, args, env, options.onSpawned); } else { return Promise.reject(err); }})
        .catch(err => { if (err.code === 'ECANCELED') {
     return Promise.reject(new UserCanceled()); } else { return Promise.reject(err); }})
        .catch(err => { if (err.systemCode === 1223) {
     return Promise.reject(new UserCanceled()); } else { return Promise.reject(err); }})
  ...
  private loadDynamicExtension(extensionPath: string,
                               ...
                               bundled: boolean): IRegisteredExtension {
    // Try common entry filenames
    const knownFormats = ['index.js','index.ts','main.js','main.ts','index.cjs','index.mjs','main.cjs','main.mjs'];
    let indexPath: string | undefined = knownFormats
  ...
  private loadDynamicExtensions(extension: { path: string, bundled: boolean },
    ...
    const res = fs.readdirSync(extension.path)
      ...
      .reduce((prev: { [id: string]: IRegisteredExtension }, name: string) => {
        ...
        return prev;
      }, {} as { [id: string]: IRegisteredExtension });
