...
export function registerProtocolHandler(
  ...
  const haveToRegister =
    ...
    !isDefaultProtocolClient(options.protocol, options.userDataPath);

  if (options.setAsDefault) {
    setDefaultProtocolClient(options.protocol, options.userDataPath);
  }
 ...
 */
export function deregisterProtocolHandler(
  protocol: string,
  userDataPath?: string,
): void {
  if (process.platform === "linux") {
  ...
  removeDefaultProtocolClient(protocol, userDataPath);
}
