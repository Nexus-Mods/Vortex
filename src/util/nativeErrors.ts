...
export function decodeSystemError(err: Error, filePath: string): IDecoded {
  ...
  if (code === 225) {
    ...
    [362, 383, 388, 390, 395, 396, 404].includes(code) ||
    (code === 1359 && (filePath ?? "").toLowerCase().includes("onedrive"))
  ) {
    return {
      title: `OneDrive error (${code})`,
      message:
        'The file "{{filePath}}" is stored on a cloud storage drive ' +
    ...
    return {
      ...
      message:
        'Windows reported an error message regarding "{{filePath}}" that indicates ' +
    ...
    return {
      ...
      message:
        'The file "{{filePath}}" is currently not accessible. If this is a ' +
    ...
    return {
      ...
      message:
        "The operation failed with an internal (internal to windows) error. " +
