 ...
 * Replace with:
 */
export const promiseMap = async <T, R>(array: T[], mapperFunction: (item: T, index: number) => Promise<R>): Promise<R[]> => {
 ...
 * Replace with:
 */
export const promiseMapSeries = async <T, R>(array: T[], mapperFunction: (item: T, index: number) => Promise<R>): Promise<R[]> => {
  const results: R[] = [];
  for (let i = 0; i < array.length; i++) {
    results.push(await mapperFunction(array[i], i));
  }
  return results;
};
