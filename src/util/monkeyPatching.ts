...
function appyMonkeyPatches() {
  monkeyPatch(Date, 'toLocaleString', function (orig, locales?: string | string[], options?: Intl.DateTimeFormatOptions) {
    try {
      return orig.apply(this, [locales, options]);
    } catch (err) {
      if ((err instanceof RangeError) && (err.message.indexOf('Unsupported time zone specified') !== -1)) {
  ...
  monkeyPatch(Date, 'toLocaleDateString', function (orig, locales?: string | string[], options?: Intl.DateTimeFormatOptions) {
    try {
      return orig.apply(this, [locales, options]);
    } catch (err) {
      if ((err instanceof RangeError) && (err.message.indexOf('Unsupported time zone specified') !== -1)) {
  ...
  monkeyPatch(Date, 'toLocaleTimeString', function (orig, locales?: string | string[], options?: Intl.DateTimeFormatOptions) {
    try {
      return orig.apply(this, [locales, options]);
    } catch (err) {
      if ((err instanceof RangeError) && (err.message.indexOf('Unsupported time zone specified') !== -1)) {
