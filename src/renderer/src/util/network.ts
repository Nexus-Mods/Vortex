...
export function rawRequest(apiURL: string, options?: IRequestOptions): Promise<string | Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const get = parsed.protocol === "http:" ? getHTTP : getHTTPS;
...
export function request(
  ...
  const reqFunc = parsed.protocol === "http:" ? requestHTTP : requestHTTPS;
      ...
      (res) => {
        ...
        res
          ...
          .on("end", () => {
            if (statusCode !== 200) {
              return reject(
                new HttpUploadError(`Upload failed. Status Code: ${statusCode}`, statusCode ?? 0),
              );
            }
            resolve({
              body: rawData,
