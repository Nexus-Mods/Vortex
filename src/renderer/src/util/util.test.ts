...
describe("objDiff", () => {
  ...
  it("handles skip parameter as non-array", () => {
    ...
    const res = util.objDiff(lhs, rhs, "not-an-array" as unknown as string[]);
    expect(res).toEqual({
      "-a": 1,
      "+a": 10,
    });
  ...
  it("handles objects with null prototype", () => {
    const obj1 = Object.create(null) as Record<string, number>;
    obj1.a = 1;
    const obj2 = Object.create(null) as Record<string, number>;
    obj2.a = 2;
...
describeOnWindows("isPathValid", () => {
  ...
  it("can be set to allow relative paths", () => {
    const validNames = ["c:\\conx\\..\\bar.txt"];
    validNames.forEach((name) => {
...
describe("nexusModsURL", () => {
  ...
  it("supports additional parameters", () => {
    expect(
      util.nexusModsURL(["foo", "bar"], {
        campaign: util.Campaign.BuyPremium,
        parameters: ["foo=bar"],
      }),
    ).toBe(
      "https://www.nexusmods.com/foo/bar?foo=bar&utm_source=vortex&utm_medium=app&utm_campaign=buy_premium",
    );
  });
});
