...
describe("storeLookup.findByAppId", () => {
  ...
  it("resolves the entry for a known app id", async () => {
    const res = await storeLookup.findByAppId(stores, "720");
    expect(res.appid).toBe("720");
    expect(res.gameStoreId).toBe("steam");
  });

  it("resolves a real Bluebird built from a plain value", () => {
    const prom = storeLookup.findByAppId(stores, "720");
    expect(prom).toBeInstanceOf(Bluebird);
    return prom;
  });

  it("rejects GameEntryNotFound on a miss", async () => {
    await expect(storeLookup.findByAppId(stores, "99999")).rejects.toThrow(GameEntryNotFound);
  });

  it("rejects GameEntryNotFound for invalid input", async () => {
    await expect(storeLookup.findByAppId(stores, [])).rejects.toThrow(GameEntryNotFound);
  });
  ...
  it("scopes to the requested store when a store id is given", async () => {
    ...
    await expect(storeLookup.findByAppId(stores, "720", "epic")).rejects.toThrow(GameEntryNotFound);
    const res = await storeLookup.findByAppId(stores, "720", "steam");
    expect(res.gameStoreId).toBe("steam");
  });

  it("supports array app ids", async () => {
    const res = await storeLookup.findByAppId(stores, ["99999", "720"]);
    expect(res.appid).toBe("720");
  });
});

describe("storeLookup.findByName", () => {
  let stores: IGameStore[];
  ...
  it("resolves the entry for an exact name match", async () => {
    const res = await storeLookup.findByName(stores, "Epic Game");
    expect(res.gameStoreId).toBe("epic");
  });

  it("does not match on partial names", async () => {
    await expect(storeLookup.findByName(stores, "Epic")).rejects.toThrow(GameEntryNotFound);
  });

  it("rejects GameEntryNotFound on a miss", async () => {
    await expect(storeLookup.findByName(stores, "No Such Game")).rejects.toThrow(GameEntryNotFound);
  });
});

describe("storeLookup.isGameInstalled", () => {
  let stores: IGameStore[];
  ...
  it("returns the store id on a hit", async () => {
    await expect(storeLookup.isGameInstalled(stores, "720")).resolves.toBe("steam");
  });

  it("returns undefined on a miss", async () => {
    await expect(storeLookup.isGameInstalled(stores, "99999")).resolves.toBeUndefined();
  });

  it("returns undefined when the game exists in a different store", async () => {
    await expect(storeLookup.isGameInstalled(stores, "720", "epic")).resolves.toBeUndefined();
  });
});

describe("storeLookup pre-scan window", () => {
  it("rejects GameEntryNotFound against empty snapshots instead of hanging or scanning", async () => {
    const stores = [makeStore("steam", [])];
    const allGames = vi.spyOn(stores[0], "allGames");
    await expect(storeLookup.findByAppId(stores, "720")).rejects.toThrow(GameEntryNotFound);
    expect(allGames).not.toHaveBeenCalled();
  });
});

describe("storeLookup.launchGameStore", () => {
  let stores: IGameStore[];
  ...
  it("resolves immediately and launches through the store's own launch logic", async () => {
    ...
    const prom = storeLookup.launchGameStore(customStores, api, "steam", ["-arg"]);
    expect(prom).toBeInstanceOf(Bluebird);
    await prom;
    await flush();
  ...
  it("resolves immediately for an unknown store and reports the failure via notification", async () => {
    const harness = makeApiHarness();
    const api = harness.api;
    await storeLookup.launchGameStore(stores, api, "doesnotexist");
    await flush();
  ...
  it("reports a failing store launch chain via notification only", async () => {
    ...
    await storeLookup.launchGameStore(failingStores, api, "steam");
    await flush();
