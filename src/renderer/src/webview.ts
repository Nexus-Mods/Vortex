...
export const makeBrowserView = (
  ...
  return getPreloadApi().browserView.createWithEvents(src, forwardEvents, options);
};
...
export const positionBrowserView = (viewId: string, rect: Electron.Rectangle): Promise<void> => {
  return getPreloadApi().browserView.position(viewId, rect);
};

export const updateViewURL = (viewId: string, newURL: string): Promise<void> => {
