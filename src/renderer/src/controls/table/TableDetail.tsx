...
import type { IEditChoice, ITableAttribute, ValidationState } from "../../types/ITableAttribute";
...
class DetailCell extends React.Component<ICellProps, {}> {
  public shouldComponentUpdate(nextProps: ICellProps) {
    ...
    if (this.props.rowData !== nextProps.rowData || this.props.rawData !== nextProps.rawData) {
      return true;
    }
    return false;
  }
  ...
  public render(): JSX.Element {
    ...
    return (
      <div className="table-details-outer">
        ...
        <Button id="btn-minimize-menu" onClick={onToggleShow} className="btn-menu-minimize">
  ...
  private onChangeData = (rowIds: string[], attributeId: string, value: any) => {
    ...
    if (attribute.supportsMultiple === true) {
      attribute.edit.onChangeValue(
        rowIds.map((rowId) => rawData[rowId]).filter((iter) => iter !== undefined),
        value,
      );
