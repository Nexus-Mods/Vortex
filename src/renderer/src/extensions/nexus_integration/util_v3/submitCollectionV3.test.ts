...
function makeApi(): IExtensionApi {
  return {
    getState: () => ({}),
...
describe("submitCollectionV3", () => {
  ...
  describe("revision update", () => {
    it("returns id, revisionNumber, and revisionStatus from V3 response (slug unchanged)", async () => {
      const result = await submitCollectionV3(makeApi(), makeManifest(), "/tmp/file.zip", 888);
    ...
    it("updates the collection name via the dedicated PATCH before the revision upload", async () => {
      await submitCollectionV3(makeApi(), makeManifest(), "/tmp/file.zip", 888);
    ...
    it("does not patch the collection when creating a new one", async () => {
      await submitCollectionV3(makeApi(), makeManifest(), "/tmp/file.zip", undefined);
  ...
  describe("auth", () => {
    it("throws when not logged in", async () => {
      ...
      await expect(
        submitCollectionV3(makeApi(), makeManifest(), "/tmp/file.zip", undefined),
