...
export const sessionReducer: IReducerSpec = {
  reducers: {
    [actions.setLoginId as any]: (state, payload) => {
      ...
      const temp = payload === undefined ? setSafe(state, ["loginError"], undefined) : state;
      return setSafe(temp, ["loginId"], payload);
    },
    [actions.setLoginError as any]: (state, payload) => setSafe(state, ["loginError"], payload),
    ...
    [actions.setOauthPending as any]: (state, payload) => setSafe(state, ["oauthPending"], payload),
  },
  defaults: {
    loginId: undefined,
