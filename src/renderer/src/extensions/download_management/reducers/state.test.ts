...
describe("initDownload", () => {
  it("initialises a download", () => {
    const input = { files: {} };
    const result = stateReducer.reducers.INIT_DOWNLOAD(input, {
      id: "id",
      urls: ["url1", "url2"],
      modInfo: { key: "value" },
      games: ["game"],
  ...
  it("terminates if the id exists", () => {
    const input = { files: { id: { state: "started" } } };
    stateReducer.reducers.INIT_DOWNLOAD(input, {
      id: "id",
      urls: ["url1", "url2"],
      modInfo: { key: "value" },
      games: ["game"],
    });
    expect(vi.mocked(terminate).mock.calls.length).toEqual(1);
  });
});
