...
describe("testModReference", () => {
  ...
  const sampleMods = {
    skyrimse: {
      "Tweaks for TTW 1.71-77934-1-71-1739639832": createMod(
        "Tweaks for TTW 1.71-77934-1-71-1739639832",
        {
      ...
      "Animation Motion Revolution-50258-1-5-3-1664395662": createMod(
  ...
  describe("Complex matching scenarios", () => {
    ...
    it("should handle undefined mod gracefully", () => {
      ...
      expect(testModReference(undefined as unknown as IMod, reference)).toBe(false);
    });
  ...
  describe("Nullish values and unexpected types edge cases", () => {
    describe("Mod parameter edge cases", () => {
      it("should return false when mod is null", () => {
        ...
        expect(testModReference(null as unknown as IMod, reference)).toBe(false);
      });
      ...
      it("should handle mod with null attributes", () => {
        const mod: IMod = {
          ...
          attributes: null as unknown as IModAttributes,
        };
        const reference: IModReference = {
          id: "test-mod",
        };
      ...
      it("should handle mod as number type", () => {
        ...
        expect(testModReference(123 as unknown as IMod, reference)).toBe(false);
      });
      ...
      it("should handle mod as string type", () => {
        ...
        expect(testModReference("not-a-mod" as unknown as IMod, reference)).toBe(false);
      });
      ...
      it("should handle mod as boolean type", () => {
        ...
        expect(testModReference(true as unknown as IMod, reference)).toBe(false);
      });
      ...
      it("should handle mod as array type", () => {
        ...
        expect(testModReference([] as unknown as IMod, reference)).toBe(false);
      });
    });
    ...
    describe("Reference parameter edge cases", () => {
      ...
      it("should return false when reference is null", () => {
        expect(testModReference(validMod, null as unknown as IModReference)).toBe(false);
      });

      it("should return false when reference is undefined", () => {
        expect(testModReference(validMod, undefined as unknown as IModReference)).toBe(false);
      });

      it("should return false when reference is number type", () => {
        expect(testModReference(validMod, 123 as unknown as IModReference)).toBe(false);
      });

      it("should return false when reference is string type", () => {
        expect(testModReference(validMod, "not-a-reference" as unknown as IModReference)).toBe(
          false,
        );
      });

      it("should return false when reference is boolean type", () => {
        expect(testModReference(validMod, false as unknown as IModReference)).toBe(false);
      });

      it("should return false when reference is array type", () => {
        expect(testModReference(validMod, [] as unknown as IModReference)).toBe(false);
      });

      it("should handle reference with null properties", () => {
        const reference: IModReference = {
          id: null as unknown as string,
          fileMD5: null as unknown as string,
          versionMatch: null as unknown as string,
          logicalFileName: null as unknown as string,
        };
    ...
    describe("Helper function edge cases with nullish values", () => {
      it("should handle isFuzzyVersion with null input", () => {
        expect(isFuzzyVersion(null as unknown as string)).toBe(false);
      });

      it("should handle isFuzzyVersion with non-string types", () => {
        expect(isFuzzyVersion(123 as unknown as string)).toBe(false);
        expect(isFuzzyVersion(true as unknown as string)).toBe(false);
        expect(isFuzzyVersion({} as unknown as string)).toBe(false);
        expect(isFuzzyVersion([] as unknown as string)).toBe(false);
      });

      it("should return empty string when sanitizeExpression gets null input", () => {
        expect(sanitizeExpression(null as unknown as string)).toBe("");
      });

      it("should return empty string when sanitizeExpression gets undefined input", () => {
        expect(sanitizeExpression(undefined as unknown as string)).toBe("");
      });

      it("should return empty string when sanitizeExpression gets non-string types", () => {
        expect(sanitizeExpression(123 as unknown as string)).toBe("");
        expect(sanitizeExpression(true as unknown as string)).toBe("");
        expect(sanitizeExpression({} as unknown as string)).toBe("");
      });

      it("should handle coerceToSemver with null input", () => {
        expect(coerceToSemver(null as unknown as string)).toBeUndefined();
      });

      it("should handle coerceToSemver with undefined input", () => {
        expect(coerceToSemver(undefined as unknown as string)).toBeUndefined();
      });

      it("should handle coerceToSemver with non-string types", () => {
        expect(coerceToSemver(123 as unknown as string)).toBeUndefined();
        expect(coerceToSemver(true as unknown as string)).toBeUndefined();
        expect(coerceToSemver({} as unknown as string)).toBeUndefined();
        expect(coerceToSemver([] as unknown as string)).toBeUndefined();
      });
    });
    ...
    describe("Complex edge cases with mixed nullish values", () => {
      it("should return false when both mod and reference are null", () => {
        expect(testModReference(null as unknown as IMod, null as unknown as IModReference)).toBe(
          false,
        );
      });
      ...
      it("should handle both mod and reference as undefined", () => {
        expect(
          testModReference(undefined as unknown as IMod, undefined as unknown as IModReference),
        ).toBe(false);
      });
  ...
  describe("Helper functions", () => {
    describe("isFuzzyVersion", () => {
      ...
      it("should handle invalid input", () => {
        ...
        expect(isFuzzyVersion(null as unknown as string)).toBe(false);
        expect(isFuzzyVersion(undefined as unknown as string)).toBe(false);
      });
    });
