...
class Todo extends React.PureComponent<ITodoProps, {}> {
  ...
  private resolveElement(
    input: string | ((t: TFunction, props: any) => JSX.Element),
    className: string,
  ): JSX.Element {
    const { t, extensionProps } = this.props;
    return input === undefined ? null : typeof input === "string" ? (
...
class TodoDashlet extends ComponentEx<IProps, {}> {
  ...
  public shouldComponentUpdate(newProps: IProps): boolean {
    return (
      newProps.dismissAll !== this.props.dismissAll ||
  ...
  public render(): JSX.Element {
    ...
    const visibleSteps = todos.filter((step) => {
      if (steps[step.id]) {
        return false;
      }
...
export default translate(["common"])(
  connect(mapStateToProps, mapDispatchToProps)(TodoDashlet),
