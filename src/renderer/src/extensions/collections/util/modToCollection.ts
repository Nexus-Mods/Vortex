...
function ruleEnabled(rule: ICollectionModRule, mods: Record<string, IMod>, collection: IMod) {
  if (rule === undefined) {
    return false;
  }
...
export async function modToCollection(
  ...
  const bundleTags: Record<string, string> = includedMods.reduce((prev, modId) => {
    prev[modId] = shortid();
    return prev;
  }, {});
