...
export default function AddToolButton(props: IBaseProps) {
  ...
  const setToolOrder = React.useCallback(
    (newOrder: string[]) => {
      onSetToolOrder(newOrder);
    },
    [onSetToolOrder],
  );
  const addNewTool = React.useCallback(
    (e) => {
      onAddNewTool();
    },
    [onAddNewTool],
  ...
  return (
    ...
    >
      ...
      >
        {hidden.map((starter) => (
          <MenuItem key={starter.id} eventKey={starter.id} onSelect={unhide}>
            {starter.name}
          </MenuItem>
        ))}
        <MenuItem key="__add" onSelect={addNewTool}>
          {t("New...")}
        </MenuItem>
      </PortalMenu>
    </Dropdown>
