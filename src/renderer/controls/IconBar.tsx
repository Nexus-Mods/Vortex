...
class IconBar extends React.Component<IProps, { open: boolean }> {
  ...
  public render(): JSX.Element {
    ...
    if (collapse) {
      ...
      return (
        <ButtonGroup id={id} className={classes.join(" ")} style={style}>
          {moreButton}
      ...
      const groupByGroup = (
        prev: { [group: string]: IActionDefinitionEx[][] },
  ...
  private renderMenuItem = (icon: IActionDefinitionEx, index: number) => {
