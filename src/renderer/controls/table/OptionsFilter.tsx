...
function OptionsFilterComponent(props: IProps & IBoundProps) {
  ...
  const changeFilterMulti = React.useCallback(
    (newFilter: Array<{ value: any; label: string }>) => {
      onSetFilter(
        attributeId,
        newFilter.map((val) => val.value),
      );
    },
  ...
  const optionsSane = options.map((opt) =>
    opt.value === undefined ? { label: opt.label, value: dummy } : opt,
  );
