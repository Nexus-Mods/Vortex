...
class DetailCell extends React.Component<ICellProps, {}> {
  ...
  public render(): JSX.Element {
    ...
    if (attribute.customRenderer !== undefined) {
      ...
      const attrControl = attribute.customRenderer(
        attribute.supportsMultiple ? values : values[0],
        true,
        t,
        {
          onHighlight: nop,
      ...
      if (attribute.edit.onChangeValue !== undefined) {
        const readOnlyFunc = getSafe(
          attribute,
          ["edit", "readOnly"],
          (val: any) => false,
        );
        const readOnly =
          rawData[rowIds[0]] !== undefined && readOnlyFunc(rawData[rowIds[0]]);
  ...
  private renderSelect(values: any[], readOnly: boolean): JSX.Element {
    ...
    const various =
      values.find((iter) => !Object.is(iter, values[0])) !== undefined;
  ...
  private renderRO(values: any[]): JSX.Element {
    ...
    if (Array.isArray(value)) {
      ...
      return (
        <ListGroup>
          {value.map((val: any) => (
            <ListGroupItem key={`${attribute.id}-${idx++}`}>
              {val}
            </ListGroupItem>
          ))}
        </ListGroup>
...
class DetailBox extends ComponentEx<IDetailProps, { hovered: boolean }> {
  ...
  public shouldComponentUpdate(
    ...
    return (
      this.props.rowIds !== nextProps.rowIds ||
    ...
    return (
      <div className="table-details-outer">
        <div className={innerClasses.join(" ")}>
          {detailList.length > 0 ? (
            <form
              className={"table-form-details " + visClass}
              ref={this.setFormRef}
            >
  ...
  private onChangeData = (
    ...
    const attribute = this.props.attributes.find(
      (attr: ITableAttribute) => attr.id === attributeId,
    );
    if (attribute.supportsMultiple === true) {
      ...
      log(
        "error",
        "attempt to change an attribute for multiple rows that doesn't support it",
