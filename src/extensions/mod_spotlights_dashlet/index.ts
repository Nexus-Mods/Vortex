...
import { IMOTMEntry, ModSpotlightEntry, ModSpotlightEntryExt, VideoEntryType } from './types';
      ...
      res
        ...
        .on('end', () => {
          try {
            ...
            reject(new ParserError(err.statusCode, err.message, link, output));
          }
        });
    }).on('error', (e) => {
      reject(e);
...
export async function update<T extends ModSpotlightEntry>(type: VideoEntryType): Promise<ModSpotlightEntryExt[]> {
  try {
    let res: T[] = [];
    if (DEBUG_MODE) {
