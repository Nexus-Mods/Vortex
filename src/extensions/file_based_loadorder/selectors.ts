...
export const currentGameMods = createSelector(allMods, activeGameId, (inMods, gameId) =>
  inMods[gameId] ?? {});
...
export const enabledMods = createSelector(currentGameMods, currentModStateForProfile, (mods, modStateIn) => {
  const res: IModLookupInfo[] = [];
  Object.keys(mods || {}).forEach(modId => {
    const attributes = mods[modId].attributes || {};
    if (getSafe(modStateIn, [modId, 'enabled'], false)
