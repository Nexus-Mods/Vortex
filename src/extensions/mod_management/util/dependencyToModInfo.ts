 ...
 */
export function dependencyToModInfo(
  ...
  const fileSizeBytes =
    primaryLookupResult?.fileSizeBytes ||
  ...
  const modInfo: IModInfo = {
    ...
    fileVersion:
      ...
      primaryLookupResult?.fileVersion ||
    ...
    // Optional fields
    ...(primaryLookupResult?.logicalFileName && {
      logicalFileName: primaryLookupResult.logicalFileName,
    }),
    ...(primaryLookupResult?.source && { source: primaryLookupResult.source }),
    ...(primaryLookupResult?.domainName && {
      domainName: primaryLookupResult.domainName,
    }),
    ...(primaryLookupResult?.expires && {
      expires: primaryLookupResult.expires,
    }),
    ...(primaryLookupResult?.archived !== undefined && {
      archived: primaryLookupResult.archived,
    }),
 ...
 */
export function dependenciesToModInfos(
...
export function getDependencyDisplayName(dependency: IDependency): string {
  ...
  return (
    primaryLookupResult?.details?.modId ||
...
export function extractModInfoFromObject(obj: any, gameId: string): IModInfo {
  ...
  const details = {
    ...
    ...(getFirstValue(
      "homepage",
      "download.modInfo.homepage",
      "modInfo.homepage",
    ) && {
      homepage: getFirstValue(
        "homepage",
        "download.modInfo.homepage",
        "modInfo.homepage",
      ),
    ...
    ...(getFirstValue(
      "fileId",
      "download.modInfo.fileId",
      "modInfo.fileId",
    ) && {
      fileId: getFirstValue(
        "fileId",
        "download.modInfo.fileId",
        "modInfo.fileId",
      ),
