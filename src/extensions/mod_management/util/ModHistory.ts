...
class ModHistory implements IHistoryStack {
    ...
    this.mEventTypes = {
      "mod-enabled": {
        ...
        revert: {
          ...
          possible: (evt) => {
            ...
            return (
              state.persistent.mods[evt.gameId]?.[id] !== undefined &&
              profile.modState?.[id]?.enabled
            );
          },
          do: async (evt) => {
            const profile = profileById(api.getState(), evt.data.profileId);
            await profileCommands.disableMod(evt.data.profileId, evt.data.id);
      ...
      "mod-disabled": {
        ...
        revert: {
          ...
          do: async (evt) => {
            const profile = profileById(api.getState(), evt.data.profileId);
            await profileCommands.enableMod(evt.data.profileId, evt.data.id);
            api.store.dispatch(setDeploymentNecessary(profile.gameId, true));
          },
        },
      },
      "mod-uninstalled": {
        describe: (evt) =>
          api.translate("Mod was uninstalled: {{ name }}", {
            replace: evt.data,
          }),
      },
      "mod-installed": {
  ...
  public init() {
    ...
    this.mApi.onStateChange<typeof state.persistent.mods>(
      ["persistent", "mods"],
      (prev, current) => {
