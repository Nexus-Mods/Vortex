...
type FileLists = Array<{
  modId: string;
  basePath: string;
  files: IFileEntry[];
}>;

function calcHashImpl(filePath: string): PromiseBB<string> {
...
function calcHash(filePath: string, tries: number = 3): PromiseBB<string> {
...
function mergeArchive(
  ...
  return (
    fs
      .ensureDirAsync(resultPath)
      // first, unpack the base/reference archive
      .then(() =>
        fs
          .statAsync(path.join(basePath, relArcPath) + BACKUP_TAG)
      ...
      // in the mod archives
      .then(() =>
        walk(resultPath, (iterPath, stats) =>
          stats.isDirectory()
            ? PromiseBB.resolve()
            ...
            .then(() =>
              walk(outputPath, (iterPath, stats) => {
                ...
                if (
                  ...
                  isDifferentProm = PromiseBB.resolve(true);
                } else {
      ...
      .then(() =>
        ...
        api
          .openArchive(path.join(mergePath, relArcPath), { gameId: game.id })
...
function mergeMods(
  ...
  return (
    PromiseBB.mapSeries(
      mods.filter((mod) => mod.installationPath !== undefined),
      (mod) => {
        ...
        return getFileList(modPath)
          ...
          .then((fileList) =>
            PromiseBB.mapSeries(fileList, (fileEntry) => {
              if (
                ...
                if (merger !== undefined) {
                  ...
                  return getNormalizeFunc(modPath)
                    ...
                    .then(() =>
                      PromiseBB.mapSeries(
                        merger.match.baseFiles(deployedFiles),
                        (file) => {
                            ...
                            if (statRes[1]) {
                              ...
                              if (isDeployed(file.in)) {
                                ...
                                return fs
                                  ...
                                  .catch(() => null)
                                  .then(() =>
                                    fs.ensureDirAsync(path.dirname(outPath)),
                                  )
                                  .then(() =>
                                    fs
                                      .copyAsync(file.in, outPath)
      ...
      // merge archives
      .then(() =>
        PromiseBB.mapSeries(Object.keys(archiveMerges), (relPath) =>
          mergeArchive(
            api,
            game,
            relPath,
