...
function checkStagingGame(api: IExtensionApi, gameId: string, manifestGameId: string): Promise<boolean> {
...
function checkStagingFolder(api: IExtensionApi, gameId: string, manifestPath: string, configuredPath: string): Promise<boolean> {
    const t = api.translate;

    // manifestPath can be undefined if the manifest is older
    return ((manifestPath !== undefined)
        ? getNormalizeFunc(manifestPath)
        : Promise.resolve(undefined as unknown as Normalize))
        .then(normalize => {
            ...
            return getNormalizeFunc(modPaths[typeId])
                .then(normalize => {
                    if ((manifests[typeId].targetPath !== undefined) &&
                        ...
                        log('warn', 'using manifest-based purge because deployment path changed', { from: manifests[typeId].targetPath, to: modPaths[typeId] });
...
export function onGameModeActivated(api: IExtensionApi, activators: IDeploymentMethod[], newGame: string) {
        ...
        if (configuredActivatorId !== undefined) {
            if (oldActivator === undefined) {
                api.showErrorNotification(
                    ...
                    {
                        message: 'The deployment method used with this game is no longer available. ' +
                ...
                if (reason.errors.length === 0) {
                    ...
                    if (safeFB !== undefined) {
                        ...
                        api.showErrorNotification(
                            ...
                            {
                                message: 'The deployment method you had configured ({{method}}) is no longer ' +
                                    'applicable.\n' +
    ...
    initProm()
        .then(() => refreshMods(api, gameId, instPath, knownMods, (mod: IMod) => {
            ...
            modNames.forEach((name: string) => {
                if (['downloaded', 'installed'].includes(knownMods[name]?.state)) {
...
function loadOrderRulesChanged(before: IModRule[], after: IModRule[]): boolean {
    ...
    // if the rules changed it's still possible the change was only in rules unrelated to load order
...
export function onModsChanged(api: IExtensionApi, previous: IModTable, current: IModTable) {
    ...
    const empty = (input: any) => !input || (Array.isArray(input) && input.length === 0);
    const different = (lhs: any, rhs: any) => (!empty(lhs) || !empty(rhs)) && lhs !== rhs;
    const changed = (modId: string, attribute: string) =>
        different(previous[gameMode][modId]?.[attribute], current[gameMode][modId]?.[attribute]);

    const rulesOrOverridesChanged = (modId: string) =>
...
function undeploy(
    ...
    log('debug', 'undeploying single mod', { game: gameMode, modIds: mods.map(mod => mod.id).join(', ') });
    ...
    return Promise.all(Object.keys(byModTypes).map(typeId => {
        ...
        return getNormalizeFunc(deployPath)
            ...
            .then(() => Promise.all(byModTypes[typeId].map(mod =>
                activator.deactivate(path.join(stagingPath, mod.installationPath), subdir(mod), mod.installationPath))))
            .catch((err) => {
            ...
            .then(newActivation =>
                saveActivation(gameMode, typeId, state.app.instanceId, deployPath, stagingPath, newActivation, activator.id));
    }))
        .finally(() => {
            log('debug', 'done undeploying single mod', { game: gameMode, modIds: mods.map(mod => mod.id).join(', ') });
        })
...
export function onRemoveMods(
    ...
    if (gameId === undefined) {
        callback?.(new ProcessCanceled('No game id assigned to remove mods from'));
        return;
    }
    ...
    const removeMods: IMod[] = modIds
        ...
        .filter((mod: IMod | undefined): mod is IMod => mod !== undefined);

    if (removeMods.length > 0) {
        store.dispatch(startActivity('mods', `removing_${removeMods[0].id}`));
    }
        ...
        .then(() => {
            ...
            return promiseMapSeries(
                ...
                .finally(() => {
                    ...
                    if (removeMods.length > 0) {
                        store.dispatch(stopActivity('mods', `removing_${removeMods[0].id}`));
                    }
                    return api.emitAndAwait('did-remove-mods', gameId, removeMods.map(m => m.id));
                });
        });
}
...
export async function onAddMod(
    api: IExtensionApi,
    gameId: string,
    mod: IMod,
...
export async function onStartInstallDownload(
    ...
    if (download === undefined) {
        if (callback !== undefined) {
            callback(new DataInvalid('Unknown Download'));
        } else {
            api.showErrorNotification(
                'Unknown Download',
    ...
    if (download.state !== 'finished') {
        ...
        if (callback !== undefined) {
            callback(new DataInvalid(message));
        } else {
            api.showErrorNotification(
                'Download Not Ready',
    ...
    const wrappedCB = (err?: Error | null, id?: string) => {
        try {
    ...
    installManager.install(
        ...
        download.game,
        api,
        { download, choices: options?.choices },
        true,
