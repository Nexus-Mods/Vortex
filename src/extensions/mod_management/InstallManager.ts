...
import { InstallManagerModuleDownloads } from "./InstallManagerModules/InstallManagerDownloads";
 ...
 */
class InstallManager {
  /* modules extracted for code clarity */
  private moduleDownloads: InstallManagerModuleDownloads =
    new InstallManagerModuleDownloads();
    ...
    return new Bluebird<void>((resolve) => {
      const poll = () => {
        ...
        if (collectionInstallProgress?.isComplete) {
          ...
          const currentPhaseComplete = isCollectionPhaseComplete(
            api.getState(),
  ...
  private maybeAdvancePhase(sourceModId: string, api: IExtensionApi) {
    ...
    while (
      state.downloadsFinished.has(curr) &&
      ...
      const finished = Array.from(state.downloadsFinished).sort(
        (a, b) => a - b,
      );
  ...
  private downloadDependencyAsync(
    ...
    if (
      ...
      return this.moduleDownloads
        ...
        .then((res) =>
          res === undefined
            ? this.moduleDownloads.downloadURL(
                api,
                lookupResult,
                wasCanceled,
                referenceTag,
      ...
      return this.moduleDownloads
        ...
        .catch((err) => {
          ...
          if (requirement.versionMatch?.endsWith?.("+prefer")) {
            return this.moduleDownloads.downloadMatching(
              api,
              lookupResult,
              requirement.versionMatch,
              referenceTag,
