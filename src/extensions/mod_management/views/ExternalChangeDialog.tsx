...
const possibleActions: { [type: string]: IPossibleAction[] } = {
  refchange: [
    {
      key: "drop",
      text: "Revert change (use staging file)",
      allText: "Revert all changes",
    },
    {
      key: "import",
      text: "Save change (use deployed file)",
      allText: "Save all changes",
    },
    {
      key: "newest",
      text: "Use newer file",
      allText: "Use newer file for all changes",
    },
  ],
  valchange: [
  ...
  deleted: [
    {
      key: "restore",
      text: "Revert change (restore file)",
      allText: "Revert all changes",
    },
    {
      key: "delete",
      text: "Save change (delete file)",
      allText: "Save all changes",
    },
  ],
  srcdeleted: [
...
class ExternalChangeDialog extends ComponentEx<IProps, IComponentState> {
  ...
  private renderSources = (
    refChanged: IFileEntry[],
    valChanged: IFileEntry[],
    deleted: IFileEntry[],
    srcDeleted: IFileEntry[],
  ): JSX.Element => {
    const { t } = this.props;
    ...
    if (deletions.length > 1) {
      this.context.api
        .showDialog(
          ...
          {
            bbcode:
              'You\'re about to [color="red"]delete[/color] {{count}} files. ' +
              "Are you sure this is what you want?",
            parameters: {
              count: deletions.length,
            },
          },
          [{ label: "Back" }, { label: "Continue" }],
        )
        .then((result) => {
