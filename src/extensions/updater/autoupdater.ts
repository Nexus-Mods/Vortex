...
function updateWarning() {
  ...
  void showMessageBox({
    ...
    message:
      "A critical update has been downloaded and needs installing. " +
...
function setupAutoUpdate(api: IExtensionApi) {
  ...
  if (currentVersion.prerelease.length > 0 && updateChannel === "stable") {
    log(
      "info",
      "current version is a pre-release and current channel is stable, setting update channel to beta",
    );
    ...
    api.sendNotification({
      ...
      type: "info",
      message:
        "You are running a beta version of Vortex so auto update settings have been " +
  ...
  const queryUpdate = (updateInfo: UpdateInfo): Promise<void> => {
    return new Promise<void>((resolve, reject) => {
      ...
      if (
        ...
        log(
          "info",
          `${updateInfo.version} is a patch update from ${autoUpdater.currentVersion.version} so we need to force download`,
        );
      ...
      if (
        ...
        log(
          "info",
          `${updateInfo.version} is greater than ${autoUpdater.currentVersion.version} so this is an upgrade.`,
        );
          ...
          actions: [
            {
              ...
              action: (dismiss) => {
                return api.showDialog(
                  ...
                  {
                    htmlText:
                      typeof filteredReleaseNotes === "string"
                  ...
                  [
                    {
                      label: "Close",
                      action: () => log("debug", "User closed dialog"),
                    },
                    /*{ label: 'Ignore', action: () => {                  
        ...
        log(
          "info",
          `${updateInfo.version} is less than ${autoUpdater.currentVersion.version} so this is a downgrade.`,
        );
...
Are you sure you want to downgrade?`,
  ...
  autoUpdater.on("update-not-available", () => {
    log(
      "info",
      `Installed version is up to date using the ${updateChannel} channel.`,
    );
  ...
  autoUpdater.on("update-available", (info: UpdateInfo) => {
    ...
    if (
      ...
      log(
        "info",
        "installed version newer than the available update, check if this is a beta",
      );
          ...
          actions: [
            {
              ...
              action: (dismiss) => {
                api.showDialog(
                  "info",
                  "Update can't be installed automatically",
                  {
                    text:
                      "An update for Vortex is available but it can't be installed automatically because " +
  ...
  ipcMain.on(
    "set-update-channel",
    (event, channel: UpdateChannel, manual: boolean) => {
      try {
