...
export default function ToolEditDialog(props: IProps) {
  ...
  const { onSetGameParameters, onAddTool, onEditEnv } = mapDispatchToProps(
  ...
  const removeEnv = React.useCallback(
    (key: string) => {
  ...
  const handleChangeParameters = React.useCallback(
    (key, value) => {
  ...
  const onShowError = React.useCallback(
    (message: string, err: Error, allowReport: boolean) => {
      context.api.showErrorNotification(message, err, { allowReport });
    },
    [context],
  );

  const onUpdateImage = React.useCallback(
    (filePath: string) => {
  ...
  const setStartEvent = React.useCallback(
    (mode: "nothing" | "hide" | "hide_recover" | "close") => {
  ...
  return (
    <Modal show={true} onHide={onClose} id="tool-edit-dialog">
      <Modal.Header>
        <Modal.Title>{realName}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form horizontal>
          <FormTextItem
          ...
          <FormGroup validationState={haveEnvironment ? "warning" : undefined}>
            <Col sm={3}>
          ...
          <FormGroup>
            <Col sm={3}>
              <ControlLabel>
                ...
                <More id="on-start" name={t("On Start behavior")}>
                  {t(
                    "Use these options if you don't want to leave the Vortex window " +
            ...
            <Col sm={9}>
              ...
              >
                <ToggleButton value="nothing">{t("Nothing")}</ToggleButton>
                <ToggleButton value="hide">{t("Hide Vortex")}</ToggleButton>
                <ToggleButton value="hide_recover">
                  {t("Hide Vortex, restore when closed")}
                </ToggleButton>
                <ToggleButton value="close">{t("Close Vortex")}</ToggleButton>
              </ToggleButtonGroup>
            </Col>
          </FormGroup>
          <FormGroup>
            ...
            <Col sm={12}>
              ...
              >
                ...
                <More id="run-detached" name={t("Run detached")}>
                  {t(
                    "Run the tool as a detached process (default). When you disable this " +
      ...
      <Modal.Footer>
        <Button id="cancel-tool-btn" tooltip={t("Cancel")} onClick={onClose}>
          {t("Cancel")}
        </Button>
        <Button id="apply-tool-btn" tooltip={t("Save")} onClick={saveAndClose}>
          {t("Save")}
        </Button>
      </Modal.Footer>
    </Modal>
