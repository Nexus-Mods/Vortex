...
class ProfileItem extends ComponentEx<IProps, IComponentState> {
  ...
  public render(): JSX.Element {
    ...
    try {
      logo =
        hasProfileImage || game === undefined
          ? this.imagePath
          : path.join(game.extensionPath, game.logo);
    } catch (err) {
      log("warn", "failed to identify profile logo path", {
        profile,
        hasProfileImage,
      });
    }
      ...
      <div className={classes.join(" ")} style={{ display: "flex" }}>
        <div style={{ flex: "1 1 0" }}>
          <div
            ...
            style={{
              background: `url('${pathToFileURL(logo).href}?${counter}')`,
            }}
    ...
    return (
      <React.Fragment key={id}>
        <div className="profile-feature-name">
          <a className="fake-link" title={t(feature.description)} onClick={nop}>
  ...
  private changeImage = () => {
    this.context.api
      .selectFile({
        filters: [{ name: "Image", extensions: ["jpg", "gif", "png"] }],
      })
      .then((file) => {
        if (file === undefined) {
          return;
        ...
        fs.writeFileAsync(this.imagePath, img.toPNG())
          ...
          .catch((err) => {
            log("warn", "failed to get path for profile image", {
