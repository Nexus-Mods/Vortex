...
function NewsExtra(props: INewsExtraProps) {
  ...
  return (
    <div>
      <Icon name={icon} />
      {t(text, { replace: { value, count }, count })}
    </div>
  );
}
  ...
  return (
    <ListGroupItem className="rss-item">
      ...
      <div
        className="rss-image"
        style={{
          background: imageUrl !== undefined ? `url(${imageUrl})` : undefined,
        }}
      />
      <h4>
        <a href={link} onClick={openMore}>
          {name}
        </a>
      </h4>
      ...
      {extra !== undefined ? (
        <div className="rss-extras">
          {extra.map((iter, idx) => (
            <NewsExtra key={idx} t={t} item={iter} />
          ))}
        </div>
      ) : null}
    </ListGroupItem>
    ...
    <Dashlet className="dashlet-news" title={title}>
      ...
      {(items ?? []).length !== 0 ? (
        <ListGroup className="list-news">
          {items.map((msg, idx) => (
            <NewsItem key={idx} t={t} item={msg} title={title} />
          ))}
        </ListGroup>
      ) : (
        <EmptyPlaceholder
