...
import { promiseEach } from '../../../util/promise-helpers';
...
export function writeDownloadsTag(api: IExtensionApi, tagPath: string): Promise<void> {
    ...
    const writeTag = () =>
        fs.writeFileAsync(path.join(tagPath, DOWNLOADS_DIR_TAG), JSON.stringify(data), { encoding: 'utf8' });

    return writeTag().catch((err: any) => {
        if (err?.code !== 'EISDIR') {
...
function removeDownloadsMetadata(api: IExtensionApi): Promise<void> {
    ...
    return promiseEach(Object.keys(downloads), dlId => {
