...
class LoadOrderPage extends ComponentEx<IProps, IComponentState> {
  ...
  public render(): JSX.Element {
    ...
    return loading ? this.renderWait() : this.renderLoadOrderPage();
  }
  ...
  private getItemRenderer() {
    ...
    return !!activeGameEntry.itemRenderer
      ? activeGameEntry.itemRenderer
      : useDefault(activeGameEntry);
  }
  ...
  private renderLoadOrderPage(): JSX.Element {
    ...
    const difference = enabledIds.filter(
      (x) => loadOrderIds.find((mod) => mod === x) === undefined,
    );
...
const empty = {};
const defaultOpts: IItemRendererOptions = {
  listViewType: "full",
...
export default withTranslation(["common", NAMESPACE])(
  connect(mapStateToProps, mapDispatchToProps)(LoadOrderPage) as any,
