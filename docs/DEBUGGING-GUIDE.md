...
This guide covers various methods for debugging Vortex, from simple console logging to advanced debugging techniques with breakpoints and inspection tools.

---

## Table of Contents

1. [Quick Start - F5 Debugging](#quick-start---f5-debugging)
2. [Log Files](#log-files)
3. [Console Debugging](#console-debugging)
4. [VS Code Debugging](#vs-code-debugging)
5. [Chrome DevTools](#chrome-devtools)
6. [Redux DevTools](#redux-devtools)
7. [Production Build Debugging](#production-build-debugging)
8. [Native Module Debugging](#native-module-debugging)
...
The fastest way to debug Vortex is using VS Code's integrated debugger.
...
2. Press **F5** (or select "Debug Electron" from the debug menu)
3. Vortex will launch with debuggers attached to both main and renderer processes
...
- Separated `runtimeArgs` and `args` to prevent VS Code from passing duplicate arguments
...
Use the "Debug Main Process" configuration:
...
This pauses at startup, allowing you to set breakpoints before any code runs.

### Debugging Renderer Process Only

Use the "Debug Renderer Process" configuration:
...
Steps:
1. Start Vortex normally (not through debugger)
2. Launch "Debug Renderer Process" from VS Code
3. Set breakpoints in renderer code
