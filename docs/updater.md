...
| **Minor / major** | 2.6.x → 2.7.0                                                 | waits for the user               | "Vortex {v} is available to download" with [What's New] [Download]; while downloading, "Downloading Vortex {v} (p%)" with a progress bar and no buttons; then "Vortex {v} is ready to install" with [What's New] [Restart Now]. |
