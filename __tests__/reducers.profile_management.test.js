...
describe('setModEnabled', () => {
  it('sets the mod enabled', () => {
    const input = { profileId1: { modState: { modId1: { enabled: false } } } };
    const result = profilesReducer.reducers.SET_MOD_ENABLED(input, { profileId: 'profileId1', modId: 'modId1', enable: true });
  ...
  it('fails if the profile doesn\'t exist', () => {
    const input = { profileId1: { modState: { modId1: { enabled: false } } } };
    const result = profilesReducer.reducers.SET_MOD_ENABLED(input, { profileId: 'profileId2', modId: 'modId1', enable: true });
    delete result.profileId1.modState.modId1.enabledTime;
    expect(result).toEqual({ profileId1: { modState: { modId1: { enabled: false } } } });
  });
  it('affects only the right profile', () => {
    const input = { profileId1: { modState: { modId1: { enabled: false } } }, profileId2: { modState: { modId1: { enabled: false } } } };
    const result = profilesReducer.reducers.SET_MOD_ENABLED(input, { profileId: 'profileId1', modId: 'modId1', enable: true });
    delete result.profileId1.modState.modId1.enabledTime;
...
describe('setFeature', () => {
  it('sets the value for the profile feature', () => {
    const input = { profileId1: { features: { featureId1: 'value' } } };
    const result = profilesReducer.reducers.SET_PROFILE_FEATURE(input, { profileId: 'profileId1', featureId: 'featureId1', value: 'new Value' });
    expect(result).toEqual({ profileId1: { features: { featureId1: 'new Value' } } });
  });
  it('fails if the profile doesn\'t exist', () => {
    const input = { profileId1: { features: { featureId1: 'value' } } };
    const result = profilesReducer.reducers.SET_PROFILE_FEATURE(input, { profileId: 'profileId2', featureId: 'featureId1', value: 'new Value' });
    expect(result).toEqual({ profileId1: { features: { featureId1: 'value' } } });
  });
  it('affects only the right profile', () => {
    const input = { profileId1: { features: { featureId1: 'value' } }, profileId2: { features: { featureId1: 'value' } } };
    const result = profilesReducer.reducers.SET_PROFILE_FEATURE(input, { profileId: 'profileId1', featureId: 'featureId1', value: 'new Value' });
