...
TODO: Do we actually need multiple fields for this? IPluginCombined is a superset of IPlugin so
  ...
  optional
*/
export const setPluginInfo = createAction(
  "SET_PLUGIN_INFO",
  (plugins: { [id: string]: IPluginCombined }) => ({ plugins }),
);

export const setPluginFilePath = createAction(
  "SET_PLUGIN_FILE_PATH",
  (pluginId: string, filePath: string) => ({ pluginId, filePath }),
);

export const updatePluginWarnings = createAction(
  "UPDATE_PLUGIN_WARNING",
  (id: string, warning: string, value: boolean) => ({ id, warning, value }),
  uiOnlyMeta,
);

export const incrementNewPluginCounter = createAction(
  "INCREMENT_NEW_PLUGIN_COUNTER",
