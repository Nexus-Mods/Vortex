...
class PluginPersistor implements types.IPersistor {
  ...
  private filterFileData(
    input: string,
    plugins: boolean,
  ): { keys: string[]; foreignApp: boolean } {
  ...
  private deserialize(retry: boolean = false): Promise<void> {
    ...
    return phaseOne
      .then((data: Buffer) => {
        ...
        const { keys, foreignApp } = this.filterFileData(
        ...
        if (foreignApp && keys.length === 0) {
          if (this.mLoaded) {
            ...
            log("info", "plugins.txt reset by foreign application during initial load; enabling all plugins");
