...
export class FormatterReadable implements IFormatter {
  ...
  public section(title: string, content: string | string[]): string {
    ...
    return [FormatterReadable.DIVIDER, `* ${title}`, FormatterReadable.DIVIDER, content, ""].join(
      "\n",
    );
  }
...
export class FormatterBBCode implements IFormatter {
  public section(summary: string, content: string | string[]): string {
    if (Array.isArray(content)) {
      return (
        `${summary} (${content.length})` + "[br][/br]" + `${this.lines(content)}` + "[br][/br]"
      );
    } else {
      ...
      lines.push(
        formatter.section(
          ...
          formatter.preformatted(
            input.loadOrder
              .map((plugin) =>
                ownedPlugins.has(plugin.name) ? `+ ${plugin.name}` : `  ${plugin.name}`,
              )
              .join("\n"),
          ),
        ),
