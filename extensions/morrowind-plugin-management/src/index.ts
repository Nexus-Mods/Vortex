...
function refreshPlugins(api: types.IExtensionApi): Promise<void> {
  ...
  return fs
    ...
    .then((plugins) =>
      readGameFiles(path.join(discovery.path, "Morrowind.ini")).then(
        (gameFiles) => ({ plugins, gameFiles }),
      ),
    )
    .then((result) => {
...
function init(context: types.IExtensionContext) {
  context.registerMainPage("plugins", "Plugins", PluginList, {
    ...
    group: "per-game",
    visible: () =>
      selectors.activeGameId(context.api.store.getState()) === "morrowind",
    props: () => ({
      localState: reactive,
        ...
        updatePluginOrder(iniFilePath, plugins)
          .then(() =>
            updatePluginTimestamps(
              path.join(discovery.path, "Data Files"),
              plugins,
            ),
          )
          .catch((err) => {
            context.api.showErrorNotification(
              "Failed to update morrowind.ini",
              err,
              { allowReport: false },
            );
          });
