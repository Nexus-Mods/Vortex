...
export const typeDescription: { [id: string]: ITypeDescription } = {
  ...
  fomod: {
    icon: "select-install",
    tooltip: "Installed using a FOMOD installer",
  },
};
...
const gamesUsingPythonScripting = new Set(["thesims4"]);
...
export const fileTypes: { [ext: string]: IFileType[] } = {
  ...
  ".py": [
    {
      type: "executable",
      condition: (gameId) => !gamesUsingPythonScripting.has(gameId),
    },
    {
      type: "script",
      condition: (gameId) => gamesUsingPythonScripting.has(gameId),
  ...
  ".png": [
    {
      type: "texture",
      condition: (gameId) => gamesUsingImagesAsTextures.has(gameId),
    },
  ],
  ".jpg": [
    {
  ...
  ".pex": [{ type: "script" }],
  ".dll": [
    { type: "extender", condition: supportsScriptExtender },
