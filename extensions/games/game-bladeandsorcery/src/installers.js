...
function testModInstaller(files, gameId, fileName) {
  ...
  const supported =
    gameId === GAME_ID &&
    files.find((file) => path.basename(file).toLowerCase() === fileName) !==
      undefined;
  return Promise.resolve({
    supported,
    requiredFiles: files.filter(
      (f) => path.basename(f).toLowerCase() === MOD_MANIFEST,
    ),
  });
}

async function installOfficialMod(
  files,
  destinationPath,
  gameId,
  progressDelegate,
      ...
      instructions: [
        {
          ...
          value: "fatal",
          source:
            "This mod is not compatible with Blade&Sorcery version 8.4 and newer.",
        },
      ],
    };
  }
  ...
  const manifestFiles = files.filter(
    (file) => path.basename(file).toLowerCase() === MOD_MANIFEST,
  );
  ...
  const createInstructions = (manifestFile) =>
    getModName(
      ...
      const isUsedModName =
        usedModNames.find((modName) => modName === manifestModName) !==
        undefined;
      ...
      const filtered = files.filter(
        (file) =>
          (file.indexOf(rootPath) !== -1 || isEngineInject) &&
...
async function installMulleMod(
  files,
  destinationPath,
  gameId,
  progressDelegate,
