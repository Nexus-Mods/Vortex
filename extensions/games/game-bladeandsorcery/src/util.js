...
async function getJSONElement(filePath, element) {
  return fs.readFileAsync(filePath, { encoding: "utf-8" }).then((data) => {
    try {
...
async function getModName(manifestFilePath, element, ext) {
  ...
  let modName =
    path.extname(folderName) === ".installing"
  ...
  return ext !== undefined ? path.basename(modName, ext) : modName;
}

async function findGameConfig(discoveryPath) {
  const findConfig = (searchPath) =>
    fs
  ...
  try {
    ...
    if (coercedMod === null) {
      return Promise.reject(
        new util.DataInvalid("Mod manifest has an invalid GameVersion element"),
      );
    }
      ...
      actions: [
        {
          ...
          action: (dismiss) =>
            api.showDialog(
              "info",
              "Missing Game.json file",
              {
                text: api.translate(
                  "Your game copy is missing its Game.json file, please try running the game at least once, or re-install the game.",
                ),
              },
