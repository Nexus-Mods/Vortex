...
export function isSmapiInternalPath(filePath: string): boolean {
  const normalizedInternalDir = SMAPI_INTERNAL_DIRECTORY.toLowerCase().replace(
    /[-_]/g,
    "",
  );
...
export function isModCandidateValid(
  mod: types.IMod | undefined,
  entry: IFileEntry,
): mod is types.IMod {
  ...
  let bundledMods: string[] = util.getSafe(
    mod,
    ["attributes", "smapiBundledMods"],
    [] as string[],
  );
