...
async function findGame() {
  ...
  if (storeGames.length > 1)
    log(
      "debug",
      "Mutliple copies of New Vegas found",
  ...
  if (["epic", "xbox"].includes(selectedGame.gameStoreId)) {
    ...
    try {
      ...
      if (!availableLocales.length) {
        log("warn", "Could not find any recognised locale folders for New Vegas", {
          folders,
      ...
      // Only one language?
        ...
        selectedGame.gamePath = path.join(
          ...
          localeFoldersXbox[availableLocales[0]],
        );
      else {
...
function testInstaller4GBPatch(api: types.IExtensionApi) {
  return (
    ...
    gameId: string,
    archivePath?: string,
    details?: types.ITestSupportedDetails,
    ...
    const hasPatchExe = PATCH_4GB_EXECUTABLES.some((execName) =>
      lowered.includes(execName.toLowerCase()),
    );
...
function applyInstaller4GBPatch(api: types.IExtensionApi) {
  return async (
    ...
    unattended?: boolean,
    archivePath?: string,
    details?: types.IInstallationDetails,
