...
//#region extensions/games/game-bloodstainedritualofthenight/common.ts
const MOD_FILE_EXT = ".pak";
const GAME_ID = "bloodstainedritualofthenight";
const LO_FILE_NAME = "loadOrder.json";
function modsRelPath() {
	return path.default.join("BloodstainedRotN", "Content", "Paks", "~mods");
}

//#endregion
//#region extensions/games/game-bloodstainedritualofthenight/util.ts
...
//#endregion
//#region extensions/games/game-bloodstainedritualofthenight/loadOrder.ts
...
//#endregion
//#region extensions/games/game-bloodstainedritualofthenight/migrations.ts
const oldModRelPath = path.default.join("BloodstainedRotN", "Content", "Paks", "~mod");
async function migrate100(api, oldVersion) {
	if (semver.default.gte(oldVersion || "0.0.1", "1.0.0")) return Promise.resolve();
	...
	if (discovery === void 0 || discovery.path === void 0 || activator === void 0) {
		(0, vortex_api.log)("debug", "skipping bloodstained migration because no deployment set up for it");
		return Promise.resolve();
	}
	return api.awaitUI().then(() => vortex_api.fs.ensureDirWritableAsync(path.default.join(discovery.path, modsRelPath()))).then(() => api.emitAndAwait("purge-mods-in-path", GAME_ID, "", path.default.join(discovery.path, oldModRelPath))).then(() => {
		api.store.dispatch(vortex_api.actions.setDeploymentNecessary(GAME_ID, true));
	});
}

//#endregion
//#region extensions/games/game-bloodstainedritualofthenight/index.ts
const STEAM_ID = "692850";
const EPIC_ID = "a2ac59c83b704e40b4ab3a9e963fef52";
async function findGame() {
	return vortex_api.util.GameStoreHelper.findByAppId([STEAM_ID, EPIC_ID]).then((game) => game.gamePath);
}
...
function main(context) {
	context.registerGame({
		...
		requiredFiles: ["BloodstainedRotN.exe", "BloodstainedROTN/Binaries/Win64/BloodstainedRotN-Win64-Shipping.exe"],
		setup: toBlue((discovery) => prepareForModding(context, discovery)),
		requiresLauncher,
		environment: { SteamAPPId: STEAM_ID },
		details: {
			steamAppId: +STEAM_ID,
			hashFiles: ["BloodstainedRotN.exe", "BloodstainedROTN/Binaries/Win64/BloodstainedRotN-Win64-Shipping.exe"]
		}
	});
