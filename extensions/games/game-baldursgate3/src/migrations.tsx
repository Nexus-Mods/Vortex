...
export async function migrate(api: types.IExtensionApi): Promise<void> {
  ...
  const currentVersion = util.getSafe(
    api.getState(),
    ["settings", "baldursgate3", "extensionVersion"],
    "0.0.0",
  );
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              bbcode: t(
                'As of Baldur\'s Gate 3 patch 7, the "ModFixer" mod is no longer required. Please feel free to disable it.{{bl}}' +
                  ...
                  'so that the game can regenerate your "modsettings.lsx" file.',
                {
                  replace: {
                    bl: "[br][/br][br][/br]",
