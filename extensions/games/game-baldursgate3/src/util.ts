...
import {
  ...
  MOD_TYPE_LSLIB,
  GAME_ID,
  DEFAULT_MOD_SETTINGS_V8,
  DEFAULT_MOD_SETTINGS_V7,
...
export function getLatestInstalledLSLibVer(api: types.IExtensionApi) {
  ...
  return Object.keys(mods).reduce((prev, id) => {
    if (mods[id].type === "bg3-lslib-divine-tool") {
      ...
      const dl: types.IDownload = util.getSafe(
        state,
  ...
  if (modsNode) {
    for (let i = 0; i < modsNode.children[0].node.length; i++) {
      ...
      if (moduleShortDescNode) {
        ...
        const uuidAttribute = moduleShortDescNode.attribute.find(
          (attr: any) => attr.$.id === "UUID",
        );
        if (uuidAttribute) {
          uuidAttribute.$.type = "guid";
        }

        const publishHandleAtt = moduleShortDescNode.attribute.find(
          (attr: any) => attr.$.id === "PublishHandle",
        );
...
export async function extractPakInfoImpl(
...
export async function readStoredLO(api: types.IExtensionApi) {
  ...
  if (enabled.length > 0 && modNodes.length === 1) {
    ...
    if (lastWrite !== undefined && lastWrite.count > 1) {
      api.showDialog(
        "info",
        '"modsettings.lsx" file was reset',
        {
          text:
            "The game reset the list of active mods and ran without them.\n" +
