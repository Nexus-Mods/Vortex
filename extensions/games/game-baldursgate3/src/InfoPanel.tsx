...
export function InfoPanelWrap(props: IBaseProps) {
  ...
  const onSetProfile = React.useCallback(
    (profileName: string) => {
      const impl = async () => {
...
function InfoPanel(props: any) {
  ...
  return isLsLibInstalled() ? (
    <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginRight: "16px" }}>
      ...
      <div>
        {t(
          `Drag and Drop PAK files to reorder how the game loads them. Please note, some mods contain multiple PAK files.`,
        )}
      </div>
      <div>
      ...
      <h4 style={{ margin: 0 }}>{t("Import and Export")}</h4>
      <div>
      ...
      <div>
        {t(
          "Please note that any mods that are not present in the BG3MM load order file will be placed at the bottom of the load order.",
        )}
      </div>
    </div>
  ) : (
    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
      <h4 style={{ margin: 0 }}>{t("LSLib is not installed")}</h4>
      <div>
        {t(
          "To take full advantage of Vortex's Baldur\s Gate 3 modding capabilities such as managing the " +
