...
function findGame() {
  return util.GameStoreHelper.findByAppId("233860", "steam").then((game) => game.gamePath);
}
let tools = [
  {
...
function getGameVersion(discoveryPath) {
  return fs
    .readFileAsync(path.join(discoveryPath, VERSION_FILE), { encoding: "utf8" })
...
function main(context) {
  context.registerGame({
    ...
    requiredFiles: ["OgreMain_x64.dll", path.join("data", "kenshi.ico")],
