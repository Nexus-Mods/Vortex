...
function prepareForModding(discovery) {
  ...
  return fs
    .ensureDirAsync(path.join(discovery.path, modPath()))
    .then(() => fs.ensureDirAsync(soundModPath))
    .then(() => modifyConfigFile(discovery.path));
}

function isAudioModType(instructions) {
  const audioFile = instructions.find(
    (inst) => inst.type === "copy" && inst.source.endsWith(AUDIO_EXT),
  );
