...
function installContent(files, destinationPath, gameId, progressDelegate) {
  ...
  const modFiles = files.filter(
    (file) => path.extname(file).toLowerCase() === MOD_EXT,
  );
...
function testSupportedContent(files, gameId) {
  ...
  const supported =
    gameId === GAME_ID &&
    files.find((file) => path.extname(file).toLowerCase() === MOD_EXT) !==
      undefined;
...
function main(context) {
  context.registerGame({
    ...
    requiredFiles: ["Torchlight2" + extension, "ModLauncher" + extension],
