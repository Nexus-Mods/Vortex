...
function getGameVersion(gamePath, exePath) {
  ...
  return Promise.resolve(
    fileVersion !== "1.0.0.0" ? fileVersion : getFileVersionLocalized(fullPath),
...
const tools = [
  ...
  },
  {
    ...
    requiredFiles: ["sksevr_loader.exe", "SkyrimVR.exe"],
    relative: true,
    exclusive: true,
    defaultPrimary: true,
  },
...
function prepare(api, discovery) {
  ...
  api.sendNotification({
    ...
    title: "ESL Support",
    message:
      "Skyrim VR requires a mod to enable ESL support. Mod must be installed through Vortex for ESL support to work.",
    actions: [
      {
        title: "Download",
        action: () =>
  ...
  context.once(() => {
    ...
    context.api.onAsync("did-deploy", (profileId, newDeployment) => {
      ...
      const modESLEnabler = deployedFiles.find((file) =>
        file.relPath.toLowerCase().endsWith(ESL_ENABLER_LIB),
      );
