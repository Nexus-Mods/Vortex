...
function toLOPrefix(context: types.IExtensionContext, mod: types.IMod): string {
  ...
  if (loEntry === undefined) {
    ...
    const prev = util.getSafe(
      props.state,
      ["settings", "7daystodie", "previousLO", props.profile.id],
      [],
    );
...
function InfoPanel(props) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", padding: "16px" }}>
      ...
      <div>
        {t(
          "7 Days to Die loads mods in alphabetic order so Vortex prefixes " +
...
function main(context: types.IExtensionContext) {
  ...
  context.registerLoadOrder({
    ...
    usageInstructions: (() => {
      ...
      return <InfoPanelWrap api={context.api} profileId={profileId} />;
    }) as any,
  });
