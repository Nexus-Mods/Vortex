    ...
    try {
      ...
      if (["EPERM"].includes(err.code)) {
        return props.api.showDialog(
          ...
          {
            bbcode: t(
              "Vortex encountered a permissions related error while attempting " +
                ...
                "[/list]",
              { replace: { filePath: err.path, bl: "[br][/br][br][/br]" } },
            ),
          },
          [
            {
...
async function handleMergedScripts(
  props: IBaseProps,
  opType: OpType,
  dest?: string,
) {
...
export async function storeToProfile(
  api: types.IExtensionApi,
  profileId: string,
) {
...
export async function queryScriptMerges(
  api: types.IExtensionApi,
...
export async function exportScriptMerges(
  api: types.IExtensionApi,
  profileId: string,
  includedModIds: string[],
  collection: types.IMod,
      ...
      const optionalSegment =
        optional.length > 0
          ? 'Marked as "optional" but need to be marked "required":{{br}}[list]' +
            optional.map((opt) => `[*]${opt}`) +
            "[/list]{{br}}"
          : "";
      const notIncludedSegment =
        notIncluded.length > 0
          ? "No longer part of the collection and need to be re-added:{{br}}[list]" +
            notIncluded.map((ni) => `[*]${ni}`) +
            "[/list]{{br}}"
          : "";
      return api
        .showDialog(
          "question",
          "Potential merged data mismatch",
          {
            bbcode:
              "Your collection includes a script merge that is referencing mods " +
...
export async function importScriptMerges(
  api: types.IExtensionApi,
  profileId: string,
  fileData: Buffer,
) {
  ...
  const res = await api.showDialog(
    ...
    {
      text:
        "The collection you are importing contains script merges which the creator of " +
    ...
    [{ label: "Cancel" }, { label: "Import Merges" }],
...
export async function makeOnContextImport(
  api: types.IExtensionApi,
  collectionId: string,
) {
  const state = api.getState();
