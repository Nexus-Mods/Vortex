...
export function testMenuModRoot(
  ...
  const hasMenuModPattern = (filePath: string) =>
    [CONFIG_MATRIX_REL_PATH, PART_SUFFIX].some(
      (pattern) => filePath.toLowerCase().indexOf(pattern) !== -1,
    );
  const predicate = (instr) =>
    !!gameId
...
export function installMenuMod(files: string[], destinationPath: string) {
  ...
  const trimmedFiles = otherFiles.map((file) => {
    const source = file;
    let relPath = file.split(path.sep).slice(binIdx);
...
export function testSupportedMixed(
  files: string[],
  gameId: string,
