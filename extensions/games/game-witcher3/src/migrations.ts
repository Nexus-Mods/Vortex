...
export async function migrate148(
  ...
  const limitPatchMod = Object.values(mods).find(
    (mod) => mod.type === "w3modlimitpatcher" && isEnabled(mod),
  );
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          ...
          context.api.showDialog(
            "info",
            "Witcher 3 Mod Limit Patch",
            {
              text: t(
                "Due to a bug, the mod limit patch was not applied correctly. " +
...
export function getPersistentLoadOrder(
