...
export async function calcChecksum(
  ...
  const conlim = new util.ConcurrencyLimiter(20, (err) =>
    ["EBADF", "EMFILE"].includes(err["code"]),
  );
...
export async function readFNISPatches(
  api: types.IExtensionApi,
  profile: types.IProfile,
): Promise<IFNISPatch[]> {
  const state: types.IState = api.store.getState();
  ...
  try {
    const patchData = await fs.readFileAsync(
      path.join(path.dirname(tool.path), patchListName(profile.gameId)),
      { encoding: "utf-8" },
    );
    return patchData
      .split("\n")
