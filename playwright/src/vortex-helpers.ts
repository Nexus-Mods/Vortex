...
export async function waitForMainWindow(
  ...
  for (let attempts = 0; attempts < 30; attempts++) {
    ...
    if (windows.length > 1) {
      for (const window of windows) {
        ...
        if (
          windowInfo.url.includes("index.html") &&
      ...
      if (
        currentInfo.url.includes("index.html") ||
  ...
  await mainWindow.waitForFunction(
    () => {
      ...
      const hasVortexContent =
        document.body.textContent!.includes("Vortex") ||
        document.querySelector(
          '[class*="App"], [class*="application"], [data-testid]',
        ) !== null;
...
export async function launchVortex(
  testName: string = "unknown-test",
): Promise<VortexLaunchResult> {
  ...
  const userDataDir = path.join(
    appdataDir,
    `vortex_playwright_${testName}_${Date.now()}`,
  );
...
function initializeTestSession(): void {
  ...
  if (
    ...
    fs.readFileSync(sessionFile, "utf8") !== currentPid
  ) {
    resetRunCounter();
