...
export function createChromePreferences(userDataDir: string): void {
  ...
  console.log('Created Chrome Preferences with NXM protocol handler pre-allowed');
  ...
  console.log('Created Chrome Local State with NXM protocol auto-launch policy');
}
 ...
 */
export async function launchRealChrome(port: number = 9222): Promise<ChromeBrowserInstance> {
 ...
 */
export async function closeRealChrome(instance: ChromeBrowserInstance): Promise<void> {
  // Kill Chrome process
  if (instance.process) {
