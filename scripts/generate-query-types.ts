...
const mainRequire = createRequire(path.resolve(__dirname, "..", "src", "main", "package.json"));
...
function derivePropertyName(query: ParsedQuery, type: "table" | "view"): string {
  if (query.alias) {
    return query.alias;
  }
  let name = query.name;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const q of setupQueries) {
      ...
      const descResult = await connection.runAndReadAll(`DESCRIBE db.${q.name}`);
