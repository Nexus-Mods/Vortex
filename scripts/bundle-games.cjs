...
function getExternalDependencies() {
  ...
  const builtins = [
    'path', 'fs', 'os', 'util', 'events', 'stream', 'child_process',
    'crypto', 'http', 'https', 'net', 'url', 'querystring', 'zlib',
    'buffer', 'string_decoder', 'assert', 'tty', 'readline', 'worker_threads'
  ];

  return [...new Set([...deps, ...devDeps, ...builtins, 'vortex-api', 'electron'])];
}
  ...
  const results = await Promise.allSettled(
    gameDirs.map(async (gameDir) => {
      ...
      try {
        ...
        const jsFiles = await glob('**/*.js', { cwd: gamePath, ignore: 'index.js' });
        for (const file of jsFiles) {
          const tsFile = file.replace('.js', '.ts');
          const tsxFile = file.replace('.js', '.tsx');
          if (await fs.pathExists(path.join(gamePath, tsFile)) ||
