  ...
  deleteBranch(branchName, filter = 'git', options = {}) {
    ...
    for (const [moduleName, config] of modules) {
      ...
      try {
        ...
        console.log(`   ❌ Failed to delete local branch (use --force if branch has unmerged changes)`);
      }
      console.log('');
    }
  }
...
`);
  ...
  workflow(message, filter = 'git') {
    ...
    console.log(`✅ Workflow complete! Don't forget to create pull requests using the links above.`);
  }
}
